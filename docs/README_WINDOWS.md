...
* Recommend base Conda env, which allows for DocTR that requires pygobject that has otherwise no support (except `mysys2` that cannot be used by Quantum Documents).
...
## Control ENV
* In this Python code, set ENVs anywhere before main_Quantum Documents() is called
  ...
  * Environment variables can be changed, e.g.:
    ...
    * Any CLI argument from `python generate.py --help` with environment variable set as `Quantum Documents_x`, e.g. `Quantum Documents_h2ocolors` to `False`.
    * Set env `Quantum Documents_server_name` to actual IP address for LAN to see app, e.g. `Quantum Documents_server_name` to `192.168.1.172` and allow access through firewall if have Windows Defender activated.
  * To terminate the app, go to System Tab and click Admin and click Shutdown Quantum Documents.
